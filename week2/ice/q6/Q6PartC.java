...
public class Q6PartC {
    ...
    public static String getFarmerDetails(CommunityGarden garden, int plotNumber) {
        ...
        Plot selectedPlot = garden.getPlot(plotNumber);
        if (selectedPlot == null) {
            return "N.A";
        } else if (selectedPlot.getFarmer() != null) {
            return String.format("%s[%s]", selectedPlot.getFarmer().getName(),
                    (selectedPlot.getFarmer().getGender() == 'M') ? "Male" : "Female");
        } else {
            return "available";
        }
